...
public class MemoryAndManipulation
{
    ...
    public void Draw_reads_current_value()
    {
        ...
        Signal.From<Flow>(flow).SetArtery(latch).Pulse(Flow.Continue);
...
It can be seen in the example at the top, but here's another one, showing a more focused usage:")]
    ...
    private static Func<Flow, Flow<Flow>> Pulse_State_overloads_example()
    {
        static Flow<Flow> flow(Flow _) =>
    ...
    [CodeSnippet]
    [CodeRemove("return")]
    private static Func<Flow, Flow<Flow>> Type_matters_using_records_flow()
    {
        return _ =>
